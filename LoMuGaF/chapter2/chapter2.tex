...
\section{Konzepte}
  ...
  keinen Server. Weil durch einen Serverbasierten Ansatz der Zustand des Spiels nur auf dem Server organisiert wird ist es eine
  gewisse Herausforderung bei einem Peer-to-Peer Ansatz alle Gerät synchron zu
  halten. Der Entwickler hat nur Objekte, über die er die Kommunikation
  realisieren muss. Dabei handelt es sich zum einen um Objekte, die die eingehenden Signale behandeln und zum anderen Objekte um Nachrichten an die anderen Spieler zu senden.
...
\end{wrapfigure}
  ...
  Wie in der Abbildung ~\ref{fig:emitter} zu sehen, hat jedes der Geräte zum
  einen einen Handler sowie einen Emitter.
  Der Emitter ist das Objekt welches die Signale an andere Geräte rausschickt und der Handler ist somit das Objekt, welches auf jedem Gerät die eingehende Nachricht behandelt. In den meisten Anwendungsfällen haben die
  Handler auf allen Geräten die selbe Impelementierung. Zusätzlich wird eine
  Session verwendet um die einzelnen Spielsitzungen oder Spielrunden in sich
...
\end{figure}
 ...
 müssen, um zu garantieren dass alle den selben Zustand haben. Jedoch ist es
...
\begin{lstlisting}[caption=Android Manifest XML]
...
entweder Methoden auf einem bestimmten Gerät aufrufen. Das ProxyObject ist das
Object was dazu benötigt wird um mit anderen Teilnehmer zu kommunizieren. Es
lässt sich auch dazu verwenden einen SignalEmmiter zu erstellen und damit
Nachrichten an alle Teilnehmer zu schicken, die zu dem Bus verbunden
sind und das entsprechende Interface implementieren.
Der Bus selbst ist ein sogenannter Daemon, also ein Hintergrundservice, welcher
auf dem Gerät läuft und sich um netzwerktechnischen Aufgaben kümmert. Dieser
wird z.B. über einen Aufruf in der Anwendung gestartet und bietet durch das
BusAttachement eine Schnittstelle für Anwendungen um damit zu interagieren. 
...
\begin{lstlisting}[caption=BusInteface Annotation]
@BusInterface (name = "de.package.MyInterface")
public interface MyInterface{}
\end{lstlisting}
Das Interface wird benötigt um über das BusAttachement an die ProxyObjekte zu
...
\begin{lstlisting}[caption=SingalHandler Annotation]
@BusSignal
...
\begin{lstlisting}[caption=BusSignalHandler Annotation]
@Override
...
\begin{lstlisting}[caption=SignalEmitter]
...
\begin{lstlisting}[caption=Session erstellen]
...
\begin{lstlisting}[caption=SessionPortListener Methoden]
