...
sogenanntes kooperatives Mehrspielerspiel. Auf dem Bild sieht man z.B. dass der
Knoten mit der Nummer 6 rot markiert ist. Somit kann der Spieler diesen Knoten
...
als Bibliothek im Spielprojekt zu referenzieren. Dazu klickt man rechts auf das
Projekt und dann auf Eigenschaften. Unter den Reiter \ib{Android} gibt es unten
die Einstellung für Bibliotheken. Hier muss man die \ib{PTPLibrary} als
...
referenzieren, sodass es beim Bauen vorhanden ist.\cite{androidLib}
...
verantwortlich. Am Anfang die \ib{onCreate()} Methode aufgerufen.
\begin{lstlisting}
...
Dies wird zuerst an den \ib{MessageHandler} weitergeleitet, welcher über die
folgende Methode die Nachrichten weiterschickt.
\begin{lstlisting}
...
die Kommunikation recht einfach gehalt ist, denn es gibt nur zwei
...
darstellt. Die Methoden des Interfaces sind wie man es im vorherigen Abschnitt
schon sehen konnte:
\begin{lstlisting}
@BusSignal
...
\end{lstlisting}
Wichtig hierbei ist die Vorgabe von AllJoyn den Methoden eine richtige
Annotation hinzuzufügen, sodass die Nachrichten entsprechend gehandhabt werden
können. In diesem Fall handelt es sich um \ib{Signalmethoden}, d.h. diese
Methoden werden dazu benutzt um Nachrichten an alle Teilnehmer gleichzeitig zu
schicken. Die konkrete Implementierung dieser Methoden beim \ib{SignalHandler}
ist somit die Behandlung der eingehenden Nachrichten.
Die Implementierung der \ib{MoveNode} Methode sieht folgendermaßen aus:
\begin{lstlisting}
...
der neue Besitzer zugewiesen. Dies wird durch diese Abfrage ermittelt:
\begin{lstlisting}
...
auf die Nachricht hört, dass der Graph neu gezeichnet werden soll.
\begin{lstlisting}
@Override
